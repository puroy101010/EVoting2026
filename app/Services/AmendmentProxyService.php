...
class AmendmentProxyService
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            $assignorUser = $proxyService->validateAssignor($request->validated());
            $assigneeUser = $proxyService->validateAssignee($request->validated(), 'amendment');


            $accountToAssign = StockholderAccount::findOrFail($request->accountToAssign);
            $createdProxy = $this->createProxyAmendment($assignorUser, $assigneeUser, $accountToAssign, $request->validated());
            $createdHistory = $this->createAmendmentProxyHistory($createdProxy);
            $remarks = "<span class='fw-bold'>Assigned</span> Amendment proxy form <span class='fw-bold'>" . $createdProxy->proxyAmendmentFormNo . "</span> to <span class='fw-bold'>" . $createdProxy->assigneeName . "</span> (" . $createdProxy->assigneeEmail . ") - ID: " . $createdProxy->proxyAmendmentId;
            ...
            ActivityController::log([
                ...
                'email' => $accountToAssign->user->email,
                'proxyAmendmentId' => $createdProxy->proxyAmendmentId,
                'proxyAmendmentHistoryId' => $createdHistory->proxyAmendmentHistoryId
            ]);
    ...
    private function createAmendmentProxyHistory(ProxyAmendment $proxy): ProxyAmendmentHistory
    {
        return ProxyAmendmentHistory::create([
            ...
            'assignorName' => $proxy->assignorName,
            'assignorEmail' => $proxy->assignorEmail,
            'assigneeName' => $proxy->assigneeName,
     ...
     *
     * @param User $assignorUser The user object of the assignor
    ...
    private function createProxyAmendment(User $assignorUser, User $assigneeUser, StockholderAccount $accountToAssign, array $request): ProxyAmendment
    {
        ...
        return ProxyAmendment::create([
            ...
            'accountId' => $accountToAssign->accountId,
            'assignorId' => $assignorUser->id,
            'assigneeId' => $assigneeUser->id,
            'assigneeName' => $assigneeUser->nonMemberAccount
                ? AppHelper::normalizeString($assigneeUser->nonMemberAccount->fullName)
                : AppHelper::normalizeString($assigneeUser->authorized_signatory), // to fix the issue of null assignee name when the assignee is a non-member user without a nonMemberAccount record
            'assigneeEmail' => $assigneeUser->email,
            'assignorName' => $assignorUser->authorized_signatory,
            'assignorEmail' => $assignorUser->email,
            'createdBy' => Auth::user()->id
        ]);
    }
    ...
    public function cancel(Request $request, int $proxyId)
    {
        try {

            $proxy = ProxyAmendment::find($proxyId);
            ...
            ActivityController::log([
                ...
                'email' => $proxy->stockholderAccount->user->email,
                'proxyAmendmentId' => $proxyId,
                'proxyAmendmentHistoryId' => $history->proxyAmendmentHistoryId,
            ]);

            ProxyAmendment::where('proxyAmendmentId', $proxyId)->forceDelete();
            DB::commit();
            return response()->json(['message' => 'Amendment proxy has been cancelled successfully.'], 200);
        } catch (Exception $e) {

            AppHelper::logServerError("Error occurred while cancelling amendment proxy with ID: $proxyId", $e);
    ...
    private function logProxyAuditActivity(ProxyAmendment $proxy, string $action)
    {
