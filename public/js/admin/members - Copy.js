...
function loadOptionAssignees() {
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data.onlineAccounts && Object.keys(data.onlineAccounts).length > 0) {
        Object.entries(data.onlineAccounts).forEach(([email, name]) => {
          assigneeList += `<option value="${email}">${name} (${email})</option>`;
        });
      }
...
function populateStockholderForm(data) {
  ...
  $("#edit_member_modal [name=stockholder]").val(data["stockholder"]["stockholder"]).attr("readonly", false);
  $("#edit_member_modal [name=account_number]").val(data["stockholder"]["accountNo"]).attr("readonly", true);
  $("#edit_member_modal [name=email]").val(data["email"]).attr("readonly", false);

  const isIndividual = data["stockholder"]["accountType"] === "indv";
  const authSignatory = isIndividual ? data["stockholder"]["stockholder"] : data["stockholder"]["authorizedSignatory"];

  $("#edit_member_modal [name=auth_signatory]").val(authSignatory).attr("disabled", isIndividual);
  $("#edit_member_modal [name=account_type] option[value=" + data["stockholder"]["accountType"] + "]").prop("selected", true).attr("disabled", false).siblings().removeAttr("selected").attr("disabled", true);
  $("#edit_member_modal [name=vote_in_person]").attr("disabled", data["stockholder"]["accountType"] === "indv").find('option[value="' + data["stockholder"]["voteInPerson"] + '"]').prop("selected", true).attr("disabled", false).siblings().removeAttr("selected").attr("disabled", data["stockholder"]["accountType"] === "indv");

  $("#edit_member_modal #edit_stockholder_details_section").show();
  $("#edit_member_modal #edit_corp_rep_section").hide();
  $("#edit_member_modal #edit_stock_details_section").hide();


  // clear fields for stockholer accounts
  $("#edit_member_modal [name=suffix]").html("<option></option>").attr("disabled", true);
  $("#edit_member_modal [name=delinquent]").attr("disabled", true).find('option[value=""]').attr("disabled", false).prop("selected", true).siblings().removeAttr("selected").attr("disabled", true);
  $("#edit_member_modal [name=corp_rep]").val("").attr("disabled", true);
  $("#edit_member_modal [name=corp_rep_email]").val("").attr("disabled", true);

}

function populateCorpRepForm(data) {
  ...
  $("#edit_member_modal #edit_stockholder_details_section").hide();
  $("#edit_member_modal #edit_corp_rep_section").show();
  $("#edit_member_modal #edit_stock_details_section").show();



  // Disable vote in person if individual account type, enable if corporate account type
  $("#edit_member_modal [name=vote_in_person]").attr("disabled", true).find("option[value=" + stockholder["voteInPerson"] + "]").attr("disabled", false).prop("selected", true).siblings().removeAttr("selected").attr("disabled", true);

  // Set suffix options and disable the field since suffix cannot be edited
  $("#edit_member_modal [name=suffix]").html("<option>" + account["suffix"] + "</option>").attr("disabled", true);

  // Disable delinquent status if individual account type, enable if corporate account type and set the value based on the account's delinquent status
  $("#edit_member_modal [name=delinquent]").attr("disabled", false).find("option[value=" + account["isDelinquent"] + "]").attr("disabled", false).prop("selected", true).siblings().removeAttr("selected").attr("disabled", false);

  // Set corporate representative and corporate representative email fields and disable them if individual account type
  $("#edit_member_modal [name=corp_rep]").val(account["corpRep"]).attr("disabled", stockholder["accountType"] === "indv");

  // Set corporate representative email field and disable it if individual account type
  $("#edit_member_modal [name=corp_rep_email]").val(data["email"]).attr("disabled", stockholder["accountType"] === "indv");

  const isIndividual = stockholder["accountType"] === "indv";
  const authSignatory = isIndividual ? account["stockholder"]["stockholder"] : account["stockholder"]["authorizedSignatory"];

  // Set authorized signatory field and disable it if individual account type
  $("#edit_member_modal [name=auth_signatory]").val(authSignatory).attr("disabled", true);
}


function editMember(id) {
  $.ajax({
    url: editMemberUrl.replace(':stockholder', id),
    method: "GET",
    dataType: "json",
    beforeSend: function () {
...
function setupEditMemberModal(data, id) {
  ...
  // Toggle stock details sections based on role
  if (isCorpRep) {
    $("#edit_member_modal #stockDetailsSection").show();
    $("#edit_member_modal #corporateRepresentativeSection").show();
  } else {
    $("#edit_member_modal #stockDetailsSection").hide();
    $("#edit_member_modal #corporateRepresentativeSection").hide();
  }
...
function handleDisplayStockholder(user, counter) {
  const accountType = user["stockholder"]["accountType"] === "indv" ? "INDV" : "CORP.";

  return `<tr data-id="${user["id"]}" class="row-stockholder">
                <td class="td-padding">#${counter}</td>
                <td class="accountKey td-padding text-nowrap td-account-no">${user["stockholder"]["accountNo"]}</td>
                <td class="td-padding td-stockholder"> ${user["stockholder"]["stockholder"]}</td>
...
function handleDisplayCorpRep(user, counter) {
  const status = user["stockholder_account"]["isDelinquent"] == "1"
    ? '<span class="badge badge-danger">Delinquent</span>'
    : '<span class="badge badge-success">Active</span>';

  const stockholder = user["stockholder_account"]["stockholder"];

  return `
          <tr data-id="${user["id"]}" data-account-id="${user["stockholder_account"]["accountId"]}">
            <td class="td-padding">#${counter}</td>
            <td class="accountKey td-padding text-nowrap td-account-no">${user["stockholder_account"]["accountKey"]}</td>
            <td class="td-padding td-stockholder">${stockholder["stockholder"]}</td>
            <td class="td-padding">${stockholder["user"]["email"] || ""}</td>
            <td class="stockholder td-padding td-stockholder">${user["stockholder_account"]["corpRep"] || ""}</td>
            <td class="email td-padding">${user["email"] || "---"}</td>
            <td class="td-padding">${stockholder["accountType"] === "indv" ? "INDV" : "CORP"}</td>
