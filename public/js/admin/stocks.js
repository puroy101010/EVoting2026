...
function generateRolePermissionRows(permissions, data) {
    ...
    permissions.forEach((p) => {
        ...
        if (
            p.description &&
            p.description.length > moduleMap[module].description.length
        ) {
    ...
    Object.keys(moduleMap)
        ...
        .forEach((module) => {
            ...
            content += `<div class="corporate-permission-group"><div class="corporate-permission-group-header"><div class="d-flex align-items-center justify-content-between"><div class="d-flex align-items-center"><span class="corporate-permission-group-icon mr-3"><i class="${moduleIcon}"></i></span><div><h6 class="corporate-permission-group-title">${module.toUpperCase()}</h6></div></div><div class="corporate-permission-count"><span class="badge badge-light">${
                moduleData.permissions.length
            } permissions</span></div></div></div><div class="corporate-permission-group-items">`;

            moduleData.permissions.forEach((permission) => {
                const isGranted = permission.has_role !== false;
                const permissionId = `perm_${
                    permission.id || Math.random().toString(36).substr(2, 9)
                }`;
                content += `<div class="corporate-permission-item"><div class="d-flex align-items-center justify-content-between"><div class="corporate-permission-info"><div class="corporate-permission-name">${
                    permission.name
                }</div><div class="corporate-permission-desc">${
                    permission.description || "No description available"
                }</div></div><div class="corporate-permission-control"><label class="corporate-modern-toggle" for="${permissionId}"><input type="checkbox" id="${permissionId}" ${
                    isGranted ? "checked" : ""
                } data-permission-id="${permission.id}" data-permission-name="${
                    permission.name
                }" data-role-id="${
                    data.recordId
                }"><span class="corporate-toggle-slider"><span class="corporate-toggle-button"></span></span></label></div></div></div>`;
            });
            content += `</div></div>`;
        });
    content += `</div></div>`;
                            ...
                            <div class="small text-muted text-truncate" style="max-width:280px;">
                                ${
                                    email
                                        ...
                                        : '<i class="fas fa-envelope-slash mr-1"></i>No email'
                                }
                            </div>
                            ${
                                lastLogin
                                    ? `<div class="small text-muted">Last: ${lastLogin}</div>`
                                    : ""
                            }
                        </div>
                        ${
                            role
                                ? `<div class="small mt-1"><span class="badge badge-outline-primary">${role}</span></div>`
                                : ""
                        }
                        ${
                            user.id
                                ? `<div class="small text-muted mt-1">ID: ${user.id}</div>`
                                : ""
                        }
                    </div>
...
function initRoleUserListFilter() {
    const input = document.querySelector(
        "#corporateRightSlideModal .corporate-user-filter"
    );
    if (!input) return;

    const items = Array.from(
        document.querySelectorAll(
            "#corporateRightSlideModal .corporate-user-item[data-user-name]"
        )
    );
    const usersList = document.querySelector(
        "#corporateRightSlideModal .corporate-users-list"
    );
        ...
        if (visibleCount === 0 && query && items.length > 0) {
            if (!noResults) {
                ...
                noResultsDiv.className =
                    "no-search-results text-center text-muted py-4";
            ...
            $.ajax({
                ...
                success: (response) => {
                    ...
                    corporateModal.setHtmlContent(
                        generateRolePermissionRows(response.permissions, data)
                    );
                },
            });
        },
...
function handleUserAssignedToThisRole(data) {
    corporateModal.init({
        title: `${
            data.recordName.charAt(0).toUpperCase() + data.recordName.slice(1)
        } Users`,
...
function initPermissionToggles() {
    const toggles = document.querySelectorAll(
        '#corporateRightSlideModal input[type="checkbox"][data-permission-id]'
    );
...
function actuallySavePermissionChanges(roleId) {
    const footerButtons = document.querySelectorAll(
        "#corporateRightSlideModal button"
    );
    const toggles = document.querySelectorAll(
        '#corporateRightSlideModal input[type="checkbox"][data-permission-id]'
    );
...
function handleUsers(data) {
    if (typeof Swal !== "undefined") {
        ...
        alert(
            `View Users with Role: ${data.recordName} (ID: ${data.recordId})`
        );
    }
}
