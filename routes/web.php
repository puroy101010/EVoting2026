...
Route::get('test', function () {
    ...
    var_dump(strtolower($a) === strtolower($b)); // false
    return;
...
Route::prefix('admin')->middleware([EnsureUserIsAdmin::class])->group(function () {
    ...
    Route::get('online-accounts/{email}/stocks', [OnlineAccountController::class, 'showStocks'])->name('online-accounts.stocks');
    Route::get('online-accounts/{email}/proxies', [OnlineAccountController::class, 'showProxies'])->name('online-accounts.proxies');
    Route::resource('online-accounts', OnlineAccountController::class)->only(['index', 'update']);
    Route::resource('non-member', NonMemberController::class);
    Route::resource('admin-account', AdminController::class);

    Route::get('stockholder/load', [StockholderController::class, 'loadStockholders'])->name('stockholder.load');
    Route::get('stockholder/assignee', [StockholderController::class, 'loadAssigneeOptions']);
