  ...
  <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div id="boardOfDirectorForm" class="ballot-pages @if($amendmentEnabled === false && (!isset($bodEnabled) || $bodEnabled !== false)) active @endif @if(isset($bodEnabled) && $bodEnabled === false) d-none @endif" data-page="2" data-page-name="bod">
          <form>
        ...
        <div id="summaryForm" class="ballot-pages" data-page="3" data-page-name="summary">
          ...
          @if(!isset($bodEnabled) || $bodEnabled !== false)
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-continue" data-btn-show-page="0" data-direction="next" id="btnSumbitBallot" onclick="handlePage(this)">
...
<script>
  ...
  const BALLOT_CONFIG = {
    ...
    isAmendmentEnabled: @if($amendmentEnabled === true) true @else false @endif,
    isBodEnabled: @if(!isset($bodEnabled) || $bodEnabled !== false) true @else false @endif,
  ...
  const StockholderVotingSystem = {
    ...
    initializeNavigation() {
      ...
      $('.ballot-pages').removeClass('active').hide();
      $(`.ballot-pages[data-page=${firstPage}]`).addClass('active').show();

      // Set initial button targets
      const nextPage = this.getNextVisiblePage(firstPage);
      $('[data-direction=back]').attr('disabled', true).attr('data-btn-show-page', firstPage);
      if (nextPage !== null) {
        $('[data-direction=next]').attr('data-btn-show-page', nextPage);
      } else {
        $('[data-direction=next]').attr('data-btn-show-page', firstPage).text('Confirm and Submit');
      }
    },

    getVisiblePages() {
      return $('.ballot-pages').not('.d-none').map(function() {
        return Number($(this).attr('data-page'));
      }).get();
    },

    getNextVisiblePage(pageNumber) {
      const pages = this.getVisiblePages();
      const idx = pages.indexOf(Number(pageNumber));
      return (idx >= 0 && idx < pages.length - 1) ? pages[idx + 1] : null;
    },

    getPrevVisiblePage(pageNumber) {
      const pages = this.getVisiblePages();
      const idx = pages.indexOf(Number(pageNumber));
    ...
    processNextPageTransition(previousPageNumber, nextPageNumber) {
      this.displayLoadingOverlay();

      setTimeout(() => {
        $('[data-direction=back]').attr('disabled', previousPageNumber === null);
    ...
    processBackPageTransition(previousPageNumber, nextPageNumber) {
      this.displayLoadingOverlay();
    ...
    async navigateToPage(navigationElement) {
      ...
      const targetPageName = $(`.ballot-pages[data-page=${targetPageNumber}]`).attr('data-page-name');
      const nextPageNumber = this.getNextVisiblePage(targetPageNumber);
      const previousPageNumber = this.getPrevVisiblePage(targetPageNumber);

      if (navigationDirection === 'next') {
        // Validate the current page before navigating away
        ...
        // Generate summary whenever navigating TO the summary page (regardless of source)
        if (targetPageName === 'summary') {
          this.displayLoadingOverlay();
          try {
      ...
      if (navigationDirection === 'next') {
        if (targetPageName === 'summary') {
          // Loading overlay was shown during summary generation — hide it now
          this.hideLoadingOverlay();
          $('[data-direction=back]').attr('disabled', false);
          $('[data-direction=next]').text('Confirm and Submit');
        } else {
          this.processNextPageTransition(previousPageNumber, nextPageNumber);
        }
      } else {
        this.processBackPageTransition(previousPageNumber, nextPageNumber);
      }

      // Update navigation button targets to the next/previous visible pages
      if (nextPageNumber !== null) {
    ...
    getAmendmentVote() {
      if (!BALLOT_CONFIG.isAmendmentEnabled) {
        return [];
      }

      if (!this.validateAmendmentSelections()) {
    ...
    getAgendaVote() {
      ...
      if (!this.validateAgendaSelections()) {
        throw new Error("Unexpected error encountered. Please reload the page.");
      }
  ...
  function handleNextButton(prevPage, nextPage) {
    return StockholderVotingSystem.processNextPageTransition(prevPage, nextPage);
  }

  function handleBackButton(prevPage, nextPage) {
    return StockholderVotingSystem.processBackPageTransition(prevPage, nextPage);
  }
