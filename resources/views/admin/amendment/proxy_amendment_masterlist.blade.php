            ...
            <div class="corporate-page-header ultra-compact">
                <div class="d-flex justify-content-between align-items-start flex-wrap">
                    ...
                    <div class="mt-3 mt-md-0">
                        <button class="btn corporate-action-btn ultra-compact" id="btnExportAmendmentProxy" type="button">
                ...
                <div class="table-responsive" style="height: 750px; overflow: auto;">
                    ...
                    <div class="col-md-12 mb-5">
                        <ol class="breadcrumb float-sm-right dflex-button">
                            <li class="breadcrumb-item"><a class="bod-filter" data-filter="all" href="{{asset('admin/amendment-proxy/masterlist?filter=all')}}">All</a></li>
                            <li class="breadcrumb-item"><a class="bod-filter" data-filter="multiple issuance" href="{{asset('admin/amendment-proxy/masterlist?filter=multiple issuance')}}">Multiple Issuance</a></li>
                            <li class="breadcrumb-item"><a class="bod-filter" data-filter="cancelled" href="{{asset('admin/amendment-proxy/masterlist?filter=cancelled')}}">Cancelled Only</a></li>
                            <!-- <li class="breadcrumb-item"><a class="bod-filter" data-filter="unverified" href="{{asset('admin/amendment-proxy/masterlist?filter=unverified')}}">Unverified Only</a></li> -->
                        </ol>
                    </div>
                    ...
                    <table class="table corporate-table ultra-compact" id="proxy_holders_table">
                        ...
                        <tbody>
                            ...
                            <tr data-id="{{ $proxy['id'] }}">
                                ...
                                <td>{{$proxy['account'] }} <br> {{ $proxy['stockholder'] }}</td>
                                <td>{{ $proxy['proxyAmendmentFormNo'] }}</td>
                                <td>{{ $proxy['assignee'] }} <br> {{ $proxy['assigneeEmail'] }}</td>
                                <td>{{ $proxy['assignor'] }}</td>
                                <td>{{ $proxy['status'] }}</td>
                                <td>{{ $proxy['remarks'] }}</td>
...
<script>
    $(document).ready(function() {
        ...
        $('.admin-nav-amendment-proxy-holders-masterlist').addClass('active');
    })
    ...
    $(document).on('click', '#btnExportAmendmentProxy', function() {
        let filter = "{{ $filter ?? '' }}";
        let exportUrl = BASE_URL + 'admin/amendment-proxy/masterlist/export?filter=' + encodeURIComponent(filter);
