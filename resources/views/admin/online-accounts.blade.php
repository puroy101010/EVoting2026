      ...
      <div class="corporate-table-container ultra-compact">
        <div class="table-responsive">
          <table id="NonmemberTable" class="table corporate-table ultra-compact">
            ...
            <tbody>
              @forelse ($onlineAccounts as $index => $onlineAccount)
              <tr data-id="{{ $onlineAccount->id }}">
                <td class="td-padding">{{ $index + 1 }}</td>
                <td class="first-name td-padding" data-first-name="{{ $onlineAccount->firstName }}">
                  @if($onlineAccount->isGM === 1)
                  <span class="badge badge-success">GM</span>
                  @endif
                  {{ $onlineAccount->firstName }}
                </td>
                <td class="middle-name td-padding">{{ $onlineAccount->middleName }}</td>
                <td class="last-name td-padding">{{ $onlineAccount->lastName }}</td>
                <td class="email td-padding">{{ $onlineAccount->email }}</td>
                <td class="account-no td-padding">{{ $onlineAccount->accountNo }}</td>
                <td class="text-center td-padding">
                  @if($onlineAccount->isActive)
